...
func NewCol(ratio float64, i ...interface{}) GridItem {
	_, ok := i[0].(Drawable) // checks if first argument is Drawable (widget)
	entry := i[0]            // assigns the first argument to entry
	if !ok {                 // if first argument is not drawable the entry is set to entire slice i
		entry = i
	}
	return GridItem{
		Type:   col,
